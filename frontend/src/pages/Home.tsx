        ...
        <div className="min-h-screen relative overflow-hidden bg-slate-50 font-sans">
            ...
            <nav className="relative z-30 flex justify-between items-center px-8 py-6 max-w-7xl mx-auto">
                ...
                <div className="flex md:hidden items-center">
                    <button
                        ...
                        className="flex flex-col justify-center items-center w-10 h-10 rounded-xl hover:bg-slate-100/80 dark:hover:bg-slate-800/80 transition-colors gap-1.5 cursor-pointer relative z-50 border border-slate-200/50 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm shadow-sm"
                    >
                        <span className={`block w-5 h-0.5 bg-slate-700 dark:bg-slate-300 rounded transition-all duration-300 ${mobileOpen ? 'translate-y-2 rotate-45' : ''}`} />
                        <span className={`block w-5 h-0.5 bg-slate-700 dark:bg-slate-300 rounded transition-all duration-300 ${mobileOpen ? 'opacity-0 scale-x-0' : ''}`} />
                        <span className={`block w-5 h-0.5 bg-slate-700 dark:bg-slate-300 rounded transition-all duration-300 ${mobileOpen ? '-translate-y-2 -rotate-45' : ''}`} />
                    </button>
                </div>
            </nav>
            ...
            {mobileOpen && (
                <div className="md:hidden fixed inset-0 z-25 flex flex-col" style={{ top: 76 }}>
                    {/* Menu panel */}
                    <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 shadow-xl px-6 py-5 flex flex-col gap-3.5 animate-slideDown">
                        {/* Native-like Quick Access Grid */}
                        <div className="grid grid-cols-3 gap-3 mb-1">
                            <Link 
                                to="/explore" 
                                onClick={closeMobile} 
                                className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-blue-50 dark:bg-slate-800/80 hover:bg-blue-100/80 dark:hover:bg-slate-700/50 border border-blue-100/50 dark:border-slate-700/30 transition-all text-center group"
                            >
                            ...
                            <Link 
                                to="/premium" 
                                onClick={closeMobile} 
                                className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/20 hover:bg-amber-100/80 dark:hover:bg-amber-950/30 border border-amber-100/50 dark:border-amber-950/10 transition-all text-center group"
                            >
                                <span className="text-2xl mb-1.5 transition-transform group-hover:scale-110">👑</span>
                            ...
                            <Link 
                                ...
                                onClick={closeMobile} 
                                className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-purple-50 dark:bg-purple-950/20 hover:bg-purple-100/80 dark:hover:bg-purple-950/30 border border-purple-100/50 dark:border-purple-950/10 transition-all text-center group"
                            >
                                <span className="text-2xl mb-1.5 transition-transform group-hover:scale-110">🏠</span>
                            ...
                            <>
                                ...
                                <div className="flex flex-col gap-2">
                                    <Link 
                                        to="/login" 
                                        onClick={closeMobile} 
                                        className="flex items-center justify-center gap-2 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold bg-slate-100/50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                                    >
                                        🔑 Connexion
                                    </Link>
                                    <Link 
